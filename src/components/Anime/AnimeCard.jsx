...
function AnimeCard({ animeObj, image_url, title, watchList, handleAddToWatchList, handleRemoveFromWatchList }) {
  ...
  return (
    <div
  className="relative w-[200px] rounded-xl overflow-hidden shadow-lg 
    bg-[#1e1e1e] hover:bg-[#2a2a2a] transition-all duration-300 
    hover:scale-[1.03] border border-pink-500/20 
    hover:shadow-pink-500/40 hover:shadow-md group"
>
  <img
    src={image_url}
    alt={title}
